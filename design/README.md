...
| [`android-overhaul-plan.md`](android-overhaul-plan.md) | **Live work, and the current priority.** Written Sep 6, 2026 from Vince's *"I think I want the lists on the app foremost"* — which turned out not to be a feature request. **The phone is a faithful client of the pre-Superlists-2.0 task core**: it still carries the three prose fields the Day page dropped on September 4, still has an Agenda screen for a surface deleted the same day, and has no `list_closed_at` at all, so it cannot show a bounded list. Nothing is broken — it shows the wrong application, correctly. **It overrides `commercial-blueprint.md` Part 9's freeze recommendation**, on Vince's August 31 call, and answers Part 9's evidence by measuring the backend cost up front rather than estimating it: the day endpoint already takes a token and already sends the line, the readback, the leftovers and appointments, so what is owed is three deliberate entries in `TOKEN_AUTHENTICATED`. Its riskiest step is manual and Vince's — a signed release will not install over the debug build, and uninstalling clears the encrypted capture queue |
| [`android-login-redesign-plan.md`](android-login-redesign-plan.md) | **Live work.** Written Sep 7, 2026 from Vince's *"avoid having this issue where there is a need to send a token to the phone"* — a stricter requirement than it sounds, since it rules out the fallback as well as the main path. **Supersedes [`android-login-plan.md`](android-login-plan.md)**, which shipped Aug 6 and is the thing being redesigned rather than a stale document. **Two independent halves, and the small one goes first**: `/me` returns scopes and expiry so a failure can name its cause, then the phone is *paired* rather than credentialled — a code travels phone → human → web, and the token travels over the phone's own connection to the device that asked. **The second factor comes free**, because the approving session already passed it; no TOTP is ever typed on a phone. Its most useful section is the one arguing `PairingRequest` against `architecture-trajectory.md` §4, because it **breaks rule 1 on purpose** — a pairing request has no owner until it is approved — and says so rather than letting it be found later. Carries one constraint found in the config rather than guessed: a polling endpoint collides with the 5r/m login rate limit |
| [`app-overhaul-plan.md`](app-overhaul-plan.md) | **Live work, and the largest thing this repository has attempted.** Written Sep 6, 2026 from Vince's *"rebuild it to just be one page"* — the whole application, both cores, one route. **Its one rule is that the page is bounded the way the list is**: seven sections that stand, seventeen things that open, against roughly thirty places to stand today. **Detail opens rather than navigates** — everything keeps a URL and loses its nav entry, which is what lets `Note` and `Person` finally get front doors while the count still falls. **D1 is answered and narrower than it looked**: `/mind/` leaves server rendering except capture and share, because the PWA scope, start_url and share target all point at capture and the phone posts to an API. **The Day page is increment 7, deliberately last**, so Superlists 2.0's fortnight survives at no cost. Its acceptance is the friction log getting shorter, not the route count falling |
| [`app-overhaul-audit-2026-09-06.md`](app-overhaul-audit-2026-09-06.md) | Written Sep 6, 2026, from Vince's *"I want to do an overhaul of the app"* — and he named **all four** readings of that word at once: two different applications, too many surfaces, how it looks, and it does not work day to day. **The whole-application successor to [`coherence-audit-2026-08-30.md`](coherence-audit-2026-08-30.md)**, which scoped itself to the task core; the half it skipped is the half that is a different application. Eight findings, and the diagnosis is **inventory rather than plumbing** — one login, one API, one palette, and about thirty places to stand. **Its most important section is the one it cannot answer**: *does not work day to day* is unreadable from a tree, and this project has been wrong there twice in writing. **A record, not a repair list** — the repairs belong to [`app-overhaul-plan.md`](app-overhaul-plan.md), written the same day |
| [`superlists-2.0-plan.md`](superlists-2.0-plan.md) | Written Sep 3, 2026, from Vince's *"not really something I'm going to use."* **The task core rebuilt around one page** — the list chosen for today, a line under it, the log of what happened, and every open line in one pool — with Areas, priorities and the Agenda retired and an `Appointment` model added. **It keeps the vision document's two rules whole** and keeps S3 and S5 at *works*. The log is a read over records that already carry a time, never a table, and every written line is a `Node`, which is the task core's first content pipe into the knowledge core. ~~Its increment 0 is a week with no code.~~ **All nine increments shipped September 3–4, 2026 and deployed twice; the plan is open because its acceptance is a fortnight of ordinary use ending around September 18.** Reviewed the day it was written and the eight edge-case points folded in, including the rename from *closed* to *bounded* and that the readback describes rather than grades. Its open decisions are its own D-list, and the plan owns `day-mockup.html` |
...
**One mockup is not a record of anything shipped: `day-mockup.html`**, September
3, 2026 — the page that replaces the Day page and the Agenda with a chosen list,
a line, a log and a pool. **Owned by
[`superlists-2.0-plan.md`](superlists-2.0-plan.md)**, written the same day; its
three moments are that plan's acceptance for four increments. Nothing in the
code cites it yet.
