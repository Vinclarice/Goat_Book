...
| [`planning-assistant-v2-plan.md`](planning-assistant-v2-plan.md) | Written Aug 19, **not started and not claimed by `roadmap.md`.** V1 finds loose ends; v2 decides what to do about them — **as the forward half of the review's ritual rather than a second surface**, which is what `kestrel` already declined and what keeps silence interpreted in one place. Twelve of fourteen elements need no generation, so it builds extractively with two named prose slots for D1, whose trigger needs eight weekly summaries and cannot fire yet. **The constraint that shapes it is not D1 but `design-concept.md`'s interactive-path rule**, Second Mind's to amend and not this tree's; the *risk* is a thin corpus, so the session inherits the null-below-a-floor discipline throughout. **Increment 6 is mostly v1 leftovers** — a name the review renders without an action, a richer question read that never had a caller worth its cost, and one enum value that would let the session record what it failed to surface. Nine increments, six decisions open; **D6 asks where the session lives given there are already two review surfaces**, and the first three increments need no decision answered |
