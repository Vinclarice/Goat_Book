...
/* ~~`SECTIONS` — Intentions, Grateful for, Happenings~~, and the form that
   saved them, **stood here until September 4, 2026.** Vince's call: they go
   from the day.

   The page had grown two ways of writing into it. The composer above puts a
   line in the log as it happens; these three asked, at the end, for the same
   day in prose — and once the log carries what happened line by line, the
   second is the one paying rent it no longer earns.

   **The fields, their data and every read of them are untouched.**
   `DailyEntry` keeps all three columns; `search_entries`, `written_in_week`
   and `propose_journal_commitments` still read them; `PATCH /api/v1/day` still
   accepts them. What left is the editor on this page. Whether the columns have
   a future is a decision Vince deferred rather than one made here — see
   `superlists-2.0-plan.md`'s *What the day no longer asks for*. */
...
export function DayRoute() {
  ...
  /* ~~The draft was seeded here~~, once per day, so an alt-tab refetch could
     not restore stored text over an edit in progress. It went with the three
     fields on September 4, 2026 — and with it the one piece of state on this
     page that could disagree with the server. */
    ...
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-8">
      ...
      {data.closing !== null && (
        <section className="space-y-1 rounded-lg border border-accent px-3 py-2">
          {/* ~~"Close the day"~~ — it asked for the day in prose, and there
              is nowhere to answer it now. What is left is what the plan's own
              mockup called this: the numbers, and one decision on each thing
              left over. */}
          <h2 className="text-sm font-bold">The day, read back</h2>
          <p className="text-sm text-muted-foreground">
