...
class NumericUserMigrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        # snapshot, so bring 'lists' and 'accounts' the rest of the way to
        # their real heads -- otherwise columns added after this test's
        # fixed migrate_to point (Item.due_date, Item.position, Item.tags,
        # User.daily_digest, ...) won't exist in the database.
        executor = MigrationExecutor(connection)
        executor.migrate(
            executor.loader.graph.leaf_nodes(app="lists")
            + executor.loader.graph.leaf_nodes(app="accounts")
        )
