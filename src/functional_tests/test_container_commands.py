...
class RemoteApproveUserSafetyTest(SimpleTestCase):
    @mock.patch("functional_tests.container_commands._exec_in_container")
    def test_remote_approval_requires_exact_host_confirmation(self, mock_exec):
        with mock.patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesMessage(RuntimeError, "approval refused"):
                container_commands.approve_user("staging.example.com", "edith")

        mock_exec.assert_not_called()

    @mock.patch("functional_tests.container_commands._exec_in_container")
    def test_confirmed_remote_approval_uses_guarded_management_command(
        self, mock_exec
    ):
        with mock.patch.dict(
            os.environ,
