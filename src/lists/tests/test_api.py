...
class TaskApiTest(TestCase):
    ...
    def make_subtask(self, text="Book flights", **extra):
        return self.request(
            "post",
            f"/api/lists/{self.list_.id}/items/",
            {"text": text, "parent": self.item.id, **extra},
        )

    def test_creates_a_subtask_opted_out_of_recurring(self):
        created = self.make_subtask(always_recurs=False).json()["data"]

        self.assertFalse(created["always_recurs"])

    def test_a_subtask_carries_forward_by_default(self):
        created = self.make_subtask().json()["data"]

        self.assertTrue(created["always_recurs"])

    def test_rejects_always_recurs_on_a_root_task(self):
        response = self.request(
            "post",
            f"/api/lists/{self.list_.id}/items/",
            {"text": "Pack", "always_recurs": False},
        )

        self.assertEqual(response.status_code, 400)

    def test_rejects_a_non_boolean_always_recurs(self):
        response = self.make_subtask(always_recurs="no")

        self.assertEqual(response.status_code, 400)
        self.assertIn("always_recurs", response.json()["errors"])

    def test_patches_always_recurs_on_a_subtask(self):
        child = self.make_subtask().json()["data"]

        updated = self.request(
            "patch", child["url"], {"always_recurs": False}
        ).json()["data"]

        self.assertFalse(updated["always_recurs"])

    def test_patching_always_recurs_on_a_root_task_conflicts(self):
        response = self.request(
            "patch", f"/api/items/{self.item.id}/", {"always_recurs": False}
        )

        self.assertEqual(response.status_code, 409)

    def test_always_recurs_counts_toward_the_one_field_per_request_rule(self):
        child = self.make_subtask().json()["data"]

        response = self.request(
            "patch", child["url"], {"always_recurs": False, "text": "Book trains"}
        )
